...
function nearbyLocation(latOrLng) {
    var smallDifference = Math.random() / 100; // should be to the 3rd decimal place

    if (Math.random() > 0.5) {
        return latOrLng + smallDifference;
    } else {
        return latOrLng - smallDifference;
    }
}

function nearbyLatLng(lat, lng) {
    var newLat, newLng;

    newLat = nearbyLocation(lat);
    newLng = nearbyLocation(lng);

    return new google.maps.LatLng(newLat, newLng);
}

function addRandomValues() {
    var i, lastLatLng, newLatLng, lat, lng;

    for (i = 0; i < 10000; i++) {
        lastLatLng = taxiData[taxiData.length - 1];
        lat = lastLatLng.lat();
        lng = lastLatLng.lng();
        newLatLng = nearbyLatLng(lat, lng);
        pointArray.push(newLatLng);
    }
